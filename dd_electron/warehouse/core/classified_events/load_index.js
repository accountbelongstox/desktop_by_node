(() => {
    ...
    class Main {
        ...
        constructor() {
            this.config_base = get_config_base()
        }
        ...
        async generateIconHTML(groupname, softConf, gid, append = false) {
            let icon_width = this.config_base.icon_width
            let basename = softConf.basename
            let img_id = this.generateGroupIconContainerId(gid)
        ...
        getListIconWidth(gListLength) {
            ...
            n = Math.ceil(n);
            let imgMargin = this.getImgMargin()
            let w = (n * (imgMargin * 2 + this.config_base.icon_width)) + 20
            return w
        }
