...
class Main {
  ...
  register(element, event_name) {
    ...
    if (event_name) {
      ...
      console.log(`Event ${event_name} not registered. Example: 'onclick=\"window.electron.register(this, 'event_name')\"'`);
    }
  }
...
class TickCase {
  ...
  set_values(data, role = undefined) {
    ...
    this.event('set_values', data)
  }

  action(action) {
    this.event('set_values', {
  ...
  bet_frompoint() {
    ...
    this.intervalID = setInterval(() => {
      if (execute_count > 0) {
        this.event('click_browserView', point)
