...
class Main {
  ...
  loadPageBaseJS() {
    ...
    this.classified_send(`htmlCase:loadPageBaseJS`, page_name, (rData) => {
      console.log(`htmlCase:loadPageBaseJS`)
      console.log(rData)
  ...
  autoExecAfterByLoaded() {
    if (!this.loadBaseSentinel) {
      ...
      let page_name = this.getPageName()
      this.classified_send(`htmlCase:autoExecAfterByLoaded`, page_name)
    }
  }
  ...
  autoExecBeforeByLoaded() {
    if (!this.lsSentinelloadBase) {
      ...
      let page_name = this.getPageName()
      this.classified_send(`htmlCase:autoExecBeforeByLoaded`, page_name)
    }
  }
  ...
  restore_default_configuration() {
    this.classified_send(`internalConfigurationFileAccessCase:getPublicConfigToHTML`, (args) => {
      let data = args.data
    ...
    const sendNextMessage = () => {
      ...
      if (this.webSocket && this.webSocket.readyState === WebSocket.OPEN) {
        try {
          ...
          if (debug_send_event) {
            console.log(`Message has been sent, cid ${cid}`);
            console.log(sendData);
          }
          sendNextMessage(); // 递归发送下一条消息
        } catch (e) {
          console.log(e);
          console.log(`Error : WebSocket interrupted, ${this.webSocketMessageQueue.length} messages have not been sent yet`);
        ...
        console.log(`WebSocket interrupted, ${this.webSocketMessageQueue.length} messages have not been sent yet`);
        this.webSocketMessageQueue.push(sendData);
        this.sendToWebsocketProcess = false;
        if (this.sendToWebsocketEvent) {
          clearInterval(this.sendToWebsocketEvent);
  ...
  getBaseConf() {
    this.classified_send(`configurationBase:getDefaultConfig`, (rData) => {
      console.log(`configurationBase:getDefaultConfig`)
...
const registerPpublicMethod = (register_name, instance) => {
  ...
  MainInstance.setFrontendPublicMethods(register_name, instance)
}
