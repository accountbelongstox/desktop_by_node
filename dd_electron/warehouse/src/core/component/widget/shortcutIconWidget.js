...
/*
getShortcutIconList(): 获取快捷方式图标列表，返回一个包含图标文件名的数组。

getShortcutIconListTotal(): 获取快捷方式图标列表的总长度。

generateIconShortcut(icons): 根据传入的图标数据生成快捷方式，包括创建 .lnk 文件，其中 icons 参数是一个包含图标信息的对象。

scanIconsDir(): 扫描指定目录下的子目录和文件，返回一个包含子目录和文件信息的对象。

updatePathsInIconCachejson(obj): 更新图标数据中的路径信息，将指定的路径替换为新路径。

mergeIconCache(iconLocal, iconsRemoteJSON): 合并本地图标数据和远程图标数据，确保本地数据包含所有远程数据。

getNoIconGenerated(localIconCache, iconsDirJSON): 获取尚未生成快捷方式的图标数据。

countSubElement(jsonObj): 统计对象中子元素的数量。

iconsDirUpToCacheJSON(localIconCache): 将新的图标数据从文件夹中添加到缓存的图标数据中。

convertRightWebPages(localIconCache): 为图标数据添加一些属性，如 ID、分组名称等。

updateIconToHTML(groupname, softConf, iconBase64): 更新图标的 HTML 显示。

genAid(basename), genGid(groupname), genImagId(basename): 生成不同类型的标识符。

readIcons(), readIconsCache(): 读取图标数据，如果尚未读取，则从本地或远程获取。

MergeRemoteIconFiles(): 从远程服务器合并图标数据。

getDefaultImageBase64Icon(), getDefaultImageFile(): 获取默认图标的 Base64 编码和文件路径。

readShortcutAsync(shortcutPath): 异步读取快捷方式文件的信息。

readIconByFile(file_path), appReadIconByFile(file_path, callback): 从文件中读取图标信息。

getIconInBase64(filePath, callback): 获取指定文件的图标，并将其转换为 Base64 编码。

readLinkFile(filePath, callback): 读取 .lnk 快捷方式文件的信息。

createShortcutAsync(shortcutPath, shortcutOptions), editShortcutAsync(shortcutPath, shortcutOptions): 异步创建或编辑快捷方式文件。

getRemoteShortcutsIconFile(), getShortcutsIconFName(), getShortcutsIconFile(): 获取远程和本地的快捷方式图标文件名和路径。

readShortcutsIconJSONByLocal(): 从本地读取快捷方式图标数据。

getIconFile(), readIconJSONCacheByLocal(), readIconJSONByLocal(): 获取本地图标数据的文件名和读取图标数据。

isCacheItemIconBase64(iconBase64): 检查图标数据是否为 Base64 编码格式。

mergeIconShort(iconsJson, shortcutsIcon): 合并图标数据和快捷方式图标数据。

saveIconJSONByLocal(config): 保存图标数据到本地文件。

generateIconJson(): 生成图标的 JSON 数据。

updateShortcut(group, basename, target, icon, base64image): 更新快捷方式的路径和图标信息。

updateShortcutPaths(dirPath, newPath, paths): 更新快捷方式的路径信息。

checkIconFileIsExists(iconConfig): 检查图标文件是否存在，更新图标数据中的文件存在状态。
*/