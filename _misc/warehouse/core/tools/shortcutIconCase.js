...
class Main {
    ...
    getShortcutIconList() {
        let shortcutIconList = fileCase.readDirectory(this.config_base.icon_dir)
    ...
    async generateIconShortcut(icons) {
        let create_icons = 0
        let icon_dir = this.config_base.icon_dir
        for (let icon_dirname in icons) {
            ...
            if (Object.keys(icon_type).length > 0) {
                ...
                fileCase.mkdirPromise(full_icondir)
                    .then(() => {
                        for (let icon_name in icon_type) {
                            ...
                            fileCase.isFileAsync(shortcut_path, (isFile) => {
                                if (!isFile) {
                                    let shortcutOption = {
        ...
        if (create_icons > 0) {
            this.event.success(`Successfully created icon: ${create_icons}.`)
        }
    }
    ...
    updatePathsInIconCachejson(obj) {
        ...
        for (let key in obj) {
            if (key === 'path' || key === 'target' || key === 'iconPath') {
                if (key) {
                    let oldPath = obj[key]
                    let drive = fileCase.getDrive(oldPath)
                    if (!includes.includes(drive) && drive) {
                        obj[key] = fileCase.replacePathByLevel(oldPath, 2, newPath);
                    }
                }
            }
        }
    ...
    async iconsDirUpToCacheJSON(localIconCache) {
        ...
        if (Object.keys(updateJSON).length) {
            ...
            let config_base = this.config_base
                ...
                for (let softname in gConf) {
                    ...
                    this.readLinkFile(lnkPath, (result) => {
                        let softConf = {
                            ...
                            basename,
                            winget_id: "",
                            isExist: fileCase.isFile(result.target),
                            source_internet: "",
                            source_local: "",
                            source_winget: "",
                            installation_method: "",
                        ...
                        if (!shortcutsIcon[basename]) {
                            this.readFileAsBase64(softConf.iconPath, (iconBase64) => {
                                ...
                                fileCase.saveJSON(jsonFile, shortcutsIcon)
                            })
                        } else {
    ...
    convertRightWebPages(localIconCache) {
        ...
        for (const groupname in localIconCache) {
            ...
            for (const basename in parent) {
                localIconCache[groupname][basename].gid = stringCase.get_id(groupname, `g`);
                localIconCache[groupname][basename].img_id = stringCase.create_id(groupname + basename)
                localIconCache[groupname][basename].groupname = groupname
                localIconCache[groupname][basename].border = utilCase.getRandomItem(borders)
            }
        }
    ...
    updateIconToHTML(groupname, softConf, iconBase64) {
        ...
        let gid = stringCase.get_id(groupname, `g`)
        let updateConf = {
            groupname,
            softConf,
            icon_width: this.config_base.icon_width,
            gid,
            img_id: `iconid_${gid}`
        }
        console.log(`index:updateIconToHtml`)
        this.event.sendToWebSocket(`index:updateIconToHtml`, updateConf)
    }
    ...
    */
    ...
    getDefaultImageBase64Icon() {
        ...
        let icon = fileCase.get_stylesheet(`img/default_app.png`)
        this.defaultImageBase64IconTemp = fileCase.readBase64ByFile(icon)
        return this.defaultImageBase64IconTemp
    }
    ...
    appReadIconByFile(file_path, callback) {
        this.app.getFileIcon(file_path, { size: 'normal' }).then((icon) => {
            try {
    ...
    readFileAsBase64(filePath, callback) {
        if (fileCase.isImageFile(filePath)) {
    ...
    getShortcutsIconFName() {
        let config_dir = fileCase.getTemp(this.getRemoteShortcutsIconFile());
        return config_dir
    }

    getShortcutsIconFile() {
        let config_dir = this.getShortcutsIconFName();
        fileCase.mkbasedir(config_dir)
        return config_dir
    }

    readShortcutsIconJSONByLocal() {
        let jsonFile = this.getShortcutsIconFile()
        return fileCase.readJSON(jsonFile)
    }

    getIconFile() {
        let config_dir = fileCase.getTemp('shortcuts.cache.json');
        fileCase.mkbasedir(config_dir)
        return config_dir
    }
    ...
    readIconJSONCacheByLocal() {
        ...
        let iconsJson = fileCase.readJSON(jsonFile)
        return iconsJson
    }
    ...
    saveIconJSONByLocal(config) {
        fileCase.saveJSON(this.getIconFile(), config)
    }

    async generateIconJson() {
        const iconJson = await this.getShortcutIcon(this.config_base.icon_dir);
    }

    async updateShortcut(group, basename, target, icon, base64image) {
        let config = this.event.get_config()
        let icon_dir = path.join(this.config_base.icon_dir, group);
        let icon_basename
        ...
        target = fileCase.replaceDir(target, config.setting_soft, 2)
        ...
        if (fileCase.existsSync(icon_path)) {
    ...
    async updateShortcutPaths(dirPath, newPath, paths) {
        ...
        files.forEach(async (file) => {
            ...
            if (stats.isDirectory()) {
                ...
                let target_include = fileCase.matchPathStartwith(target, paths)
                let icon_include = fileCase.matchPathStartwith(icon, paths)
                let workingDir_include = fileCase.matchPathStartwith(workingDir, paths)
                if (target_include) {
                    target = fileCase.pathReplace(target, target_include, newPath)
                }
                if (icon_include) {
                    icon = fileCase.pathReplace(icon, icon_include, newPath)
                }
                if (workingDir_include) {
                    workingDir = fileCase.pathReplace(workingDir, workingDir_include, newPath)
                }
                let shortcutOptions = {
                    target,
                    workingDir,
    ...
    checkIconFileIsExists(iconConfig) {
        for (let gname in iconConfig) {
            for (let basename in iconConfig[gname]) {
                let isExist = fileCase.isFile(iconConfig[gname][basename].target);
                iconConfig[gname][basename].isExist = isExist
                let is_local = iconConfig[gname][basename].is_local
                if (is_local) {
                    let gdir = path.join(this.config_base.icon_dir, gname)
                    let linkdir = path.join(gdir, basename)
                    if (!fileCase.isFile(linkdir)) {
