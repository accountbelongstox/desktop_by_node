...
class zipProcess {
  ...
  async execTask() {
    if (!this.execTaskEvent) {
      ...
      this.execTaskEvent = setInterval(() => {
        let processZipCount = winapiCase.processesCount(`7z.exe`)
        ...
        if (this.concurrentTasks >= this.maxTasks) {
          ...
          if (!fileCase.isFileLocked(zipPath)) {
  ...
  putUnZipTask(src, out, callback) {
    let token = stringCase.get_id(src)
  ...
  putQueueCallback(callback, token) {
    if (callback && !this.callbacks[token]) {
      if (!token) token = stringCase.create_id()
  ...
  putTask(src, out, token, isZip = true, callback, isQueue = true) {
    ...
    if (isZip) {
      ...
      if (fileCase.existsSync(zipPath)) {
        if (!this.mode) {
          return
        }
        if (this.mode.update) {
          let srcModiTime = fileCase.getModificationTime(src)
          let zipPathModiTime = fileCase.getModificationTime(zipPath)
          let difTime = srcModiTime - zipPathModiTime
          if (difTime < 1000 * 60) {
            return
          }
          fileCase.delete(zipPath)
      ...
      let zipSize = fileCase.getFileSize(zipPath)
      if (zipSize == 0) {
        fileCase.delete(zipSize)
      }
