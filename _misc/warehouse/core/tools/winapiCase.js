...
const utilFunc = require("./utilCase.js")
...
class WindowsAPI {
    ...
    getMainExe(callback) {
        fs.readdir(fileCase.get_root(), (err, files) => {
            if (err) {
    ...
    setstartup() {
        this.getMainExe((apps) => {
            if (apps.length) {
                let app = apps.pop()
                const updateExe = path.resolve(path.join(fileCase.get_root(), app));
    ...
    checkAdmin(callback) {
        if (!this.isAdmin()) {
            this.event.error(`请以管理员模式运行,(右键:管理员运行.)`)
            setTimeout(() => {
    ...
    waitPublicConfig(callback) {
        let public_config = this.event.get_config()
        if (!Object.keys(public_config).length > 0) {
            setTimeout(() => {
    ...
    getPrivateUserDir(dir) {
        ...
        fileCase.mkbasedir(private_dir)
        return private_dir
    }
    ...
    getPath(name, dir) {
        ...
        if (dir) {
            homedir = path.join(homedir, dir)
            if (!fileCase.isDir(homedir)) fileCase.mkdir(homedir)
        }
        return homedir
    }
    ...
    async setPathEnv(envs) {
        ...
        if (new_env.length == 0) {
            //没有新的变量，直接结束，无需设置
            this.event.log(`没有新的变量，直接结束，无需设置`)
            return
        }

        // let indexToSplit = utilFunc.array_lastchat(currentPath, "c:") + 1
        ...
        let default_python = stringCase.getDefault(public_config.default_python, '3.10');
        let default_java = stringCase.getDefault(public_config.default_java, '9');
        let default_node = stringCase.getDefault(public_config.default_node, '18');
        ...
        new_env = utilFunc.array_priority(new_env, default_version)
        ...
        if (oriLen != currentPath.length) {
            ...
            const listResult = await this.setRegistry(this.pathKey, regValue)
            return listResult
        }
    }
    ...
    getDefaultEnvVersion() {
        ...
        let envs = fileCase.scanDir(local_envdir)
        let evn = {}
    ...
    async setRegistry(regKey, value) {
        let regValue = {}
        regValue[regKey] = value
        const listResult = await regedit.putValue(regValue)
        return listResult
    }
    ...
    saveUserData(key, val) {
        ...
        let data = fileCase.readJSON(data_dir)
        data[key] = val;
        fileCase.saveJSON(data_dir, data)
    }
    ...
    exec_explorer(file_path, group, default_config, callback) {
        ...
        console.log(cmd)
        utilCase.exeBySpawn(cmd, (err, std) => {
    ...
    exec_asadmin(file_path, group, default_config, pare = '', callback) {
        ...
        if (!fs.existsSync(file_path)) {
            ...
            if (fileCase.isExecutable(file_path)) {
                let gsudo = fileCase.get_bin(`gsudo.exe`)
    ...
    isHyperVEnabled(callback) {
        utilCase.exec_cmd('dism /online /get-featureinfo /featurename:Microsoft-Hyper-V', (stdout, error, stderr) => {
    ...
    getWslDistributes(callback) {
        let cmd = 'wsl -l --quiet'
        utilCase.exec_cmd(cmd, (stdout, error) => {
            stdout = stringCase.trim(stdout)
    ...
    isWSL2Enabled(callback) {
        utilCase.exec_cmd('wsl --status', (stdout, error) => {
