...
class FileCase {
    ...
    readFromTextFile(filepath) {
        ...
        for (let line of lines) {
            ...
            obj[key] = StringCase.convertString(value);
        }
                ...
                if (this.execCountTasks >= this.maxTasks) {
                    ...
                    if (!this.isFileLocked(src) && !this.isFileLocked(out)) {
                        ...
                        if (this.isFile(src)) {
                            ...
                            if (info) {
                                this.event.success(`copying Directory :${src} to ${out}.`, true);
                            } else {
                        ...
                        if (info) {
                            this.event.success(`File is locked, requeue, file : ${src}`, true);
                        } else {
    ...
    mapNetworkDriveSync(hoststr) {
        let { url, usr, pwd } = stringCase.getShareStringUserPassword(hoststr)
        ...
        let { hostname, pathname } = stringCase.parseShareDirHostAndPath(url)
